...
**AGenUI** is an A2UI SDK that simultaneously supports iOS, Android, HarmonyOS, and Web. Built on and fully implementing the [A2UI v0.9 protocol open-sourced by Google](https://github.com/google/A2UI), it renders the streaming data of LLM-generated interactive UI in real time on mobile devices. Under the hood, it is powered by a cross-platform **shared C++ core engine**, while the three-platform rendering engines consume the component protocol dispatched by the core engine and perform drawing using native system capabilities.
...
- **High performance** — native rendering on iOS, Android, HarmonyOS, and Web, maintaining a 120 fps refresh rate in core scenarios such as page scrolling
- **25 built-in components** — 18 A2UI protocol components + 4 SDK extension components + 3 Playground example components
...
| `platforms/web/` | Web component renderer + React bridge |
| `playground/` | Four-platform demo apps for development & debugging |
...
```bash
# Build SDK (ESM + CJS + UMD)
cd platforms/web && npm run build

# Output: platforms/web/dist/
```
